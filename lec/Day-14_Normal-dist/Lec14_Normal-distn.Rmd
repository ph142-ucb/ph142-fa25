...
:::: column
```{r load-libraries, echo=F, out.width = "100%", warning=F, message=F}
library(ggplot2)
library(patchwork)
library(latex2exp)
library(dplyr)
...
```{r plot heights, out.width="100%", echo=F, warning=F, message=F}
...
observations in the distribution that lie below $x$
...
:::: column
```{r, echo=F, out.width="100%", message=F}
p1 <- ggplot(data = data.frame(x = c(-3, 3)), aes(x)) +
  stat_function(fun = dnorm, n = 101, args = list(mean = 0, sd = 1), col = "orange") +
  labs(y = "density", x = "") + 
  annotate("text", x = 0, y = 0.45, label = "mean = 0 \nsd = 1", check_overlap = T, col = "orange") +
...
- To answer this question we use the `pnorm()` function
  ...
  - The **norm** in `pnorm` stands for normal curve
