---
title: "Lecture 21: Inference in Practice"
author: "Tomer Altman"
...
* [Chapter 15](https://ebookcentral-proquest-com.libproxy.berkeley.edu/lib/berkeley-ebooks/reader.action?docID=6643934&ppg=790&c=RVBVQg) of Baldi and Moore
* Additional Resources:
  * [Learning Statistics with R](https://learningstatisticswithr.com/lsr-0.6.pdf): Type I and Type II error, section 11.2, page 330
  * [Statistical Thinking for the $21^{\text{st}}$ Century](https://statsthinking21.github.io/statsthinking21-core-site/ci-effect-size-power.html#statistical-power): Basics of power, including its relation to Type I/Type II error, and factors that affect power, sections 10.3-10.3.1
...
1. SRS
2. Population has a Normal distribution (or $n$ large)
3. $\sigma$ is known (or $n$ large enough that one can use sample SD)
...
Sometimes one still reports inference for non-random samples, treating them as if they were generated from a SRS. Given reporting some measure of uncertainty is better than not, though the inference might not reflect the true sampling distribution.
...
- Non response: Some individuals may start but not complete a survey. What happens
...
- Examine the shape of the sample's distribution using a Q-Q plot 
(ideally) or a histogram
- Use it to infer the shape of the population distribution
- Difficult to infer much if there are too few observations

### Condition 2: What is the shape of the population distribution?

- Outliers can affect tests of non-robust measures like the mean
- Double check if the outlier is "real" or an error
  - If it is real, can use other methods that aren't sensitive to outliers
...
$\mu < \mu_0$ or $\mu > \mu_0$
- You should never use $\bar{x}$ as a 
...
- We also call $\frac{\sigma}{\sqrt{n}}$ the "size of chance variation" because it 
...
- Read this [Science article](https://www.science.org/content/article/cornell-nutrition-scientist-resigns-after-retractions-and-research-misconduct-finding) about a Cornell food researcher who engaged in p-hacking
...
- You can rearrange the formula for the margin of error to solve for $n$:
...
body temperature $\mu$ for healthy adults within $\pm0.05^{\circ}F$ with 95% confidence. 
...
* The width of the confidence interval is $2\times{M}$, so here $M=0.05$, 
