...
* [Bar Plots](https://viz-ggplot2.rsquaredacademy.com/ggplot2-bar-plot)
* [Clustered Bar Charts](https://bookdown.org/rwnahhas/IntroToR/categorical-vs-categorical-clustered-bar-charts.html)
<!-- This link doesn't work anymore, and I couldn't find a new one: -->
<!-- * [Relationships in categorical data](http://digfir-published.macmillanusa.com/psbe4e/psbe4e_ch2_14.html)  -->

### Two-way tables

- Two-way stands for $2 \times 2$, as in a table with two columns and two rows
- Used to examine the relationship between two categorical variables, originally those with two levels
- Foundational to epidemiology, because of the types of variables we are often interested in

### Classic $2 \times 2$ table format
...
- We learned previously how to plot marginal distributions of categorical variables using `geom_bar()`
...
```{r visualize-conditional-bar-chart-dodged, out.width="40%"}
...
```{r visualize-conditional-bar-chart-stacked, out.width = "40%"}
ggplot(two_way, aes(x = smoking, y = percent)) + 
...
```{r shoe-dodge-reorder, out.width="40%"}
shoe_data <- shoe_data %>% 
...
```{r shoe-dodge-recol, out.width="40%"}
ggplot(shoe_data, aes(x = gender, y = percent)) + 
...
:::: column
```{r simpsons-with-marginal, out.width="80%", fig.align='center', echo=F}
...
:::: column
```{r bar-chart-simpsons-paradox, out.width="80%", echo=F}
...
:::: column
```{r bar-chart-simpsons-paradox-2, out.width="80%", echo=F}
#students, don't need to know the scale_fill_manual() function
ggplot(simp_data, aes(x = community, y = death_per_1000)) + 
  geom_bar(aes(fill = age_grp), position = "dodge", stat = "identity", col = "white") +
  theme_minimal(base_size = 15) +
